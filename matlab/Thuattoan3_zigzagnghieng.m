function Result = Thuattoan3_zigzagnghieng(showPlots, showTables)
...
%% --- STEP 4: Zig-zag nghiêng (Diagonal lawn-mower) ---
numUAV = 6;
camRadius = 10;            % bán kính footprint (m)
maxOverlap = 5;            % overlap tối đa giữa hai dải (m)
sweepSpacing = 2*camRadius - maxOverlap; % spacing chuẩn (m)
thetaDiagRange = deg2rad(linspace(30, 45, 4)); % duyệt góc nghiêng 30° → 45°
thetaStep = deg2rad(5);
thetaList = 0:thetaStep:(pi - thetaStep);
basePoint = [0 0];
costWeights = [1.0, 0.02, 0.55];
costTemplate = struct('turn',0,'length',0,'overlap',0,'total',0,'sweptArea',0,'turnCount',0);

bestConfig = struct('totalCost', inf, 'thetaBase', NaN, 'thetaDiag', NaN, ...
                    'paths', [], 'costs', [], 'subPolys', []);
...
for thetaBase = thetaList
    ...
    for thetaDiag = thetaDiagRange
        ...
        for k = 1:numUAV
            diagPathRot = generate_diagonal_lawnmower(subPolysRot{k}, sweepSpacing, thetaDiag);
            if size(diagPathRot,1) < 2
                validConfig = false;
                break;
            end
            diagPathENU = rotate_points(diagPathRot, thetaBase);
            pathWithTransit = add_transit_segments(diagPathENU, basePoint);
            subPolyENU = rotate(subPolysRot{k}, rad2deg(thetaBase), [0 0]);
            costStruct = evaluate_sweep_cost(pathWithTransit, diagPathENU, subPolyENU, camRadius, costWeights);
        ...
        fprintf('Theta_base = %5.1f deg, Theta_diag = %4.1f deg → Tổng chi phí = %.3f\n', ...
                rad2deg(thetaBase), rad2deg(thetaDiag), thetaTotal);

        if thetaTotal < bestConfig.totalCost
            bestConfig.totalCost = thetaTotal;
            bestConfig.thetaBase = thetaBase;
            bestConfig.thetaDiag = thetaDiag;
...
if ~isfinite(bestConfig.totalCost)
    error('Không tìm được cấu hình diagonal lawn-mower hợp lệ.');
end

fprintf('\nDiagonal lawn-mower tối ưu: θ_base = %.2f deg, θ_diag = %.2f deg, tổng chi phí = %.3f\n', ...
        rad2deg(bestConfig.thetaBase), rad2deg(bestConfig.thetaDiag), bestConfig.totalCost);
...
if showPlots
    figure('Color','w'); hold on; grid on; axis equal; box on;
    plot(P,'FaceColor',[0.9 1.0 0.95],'FaceAlpha',0.25,'EdgeColor',[0.2 0.5 0.4],'LineWidth',1.2);
    colors = lines(numUAV);
    ...
    xlabel('East (m)'); ylabel('North (m)');
    title(sprintf('Hình 2 – Diagonal lawn-mower (θ_b = %.1f°, θ_d = %.1f°)', ...
        rad2deg(bestConfig.thetaBase), rad2deg(bestConfig.thetaDiag)));
...
%% --- STEP 5: Chuẩn hóa kết quả diagonal ---
...
thetaInfo = struct('base_deg', rad2deg(bestConfig.thetaBase), ...
                   'diag_deg', rad2deg(bestConfig.thetaDiag));
...
overallTable = table(rad2deg(bestConfig.thetaBase), rad2deg(bestConfig.thetaDiag), bestConfig.totalCost, ...
    sum(path_length), totalRegionArea, sum(covered_area), overlap_ratio, sum(num_turns), ...
    'VariableNames', {'Theta_base_deg','Theta_diag_deg','Total_cost','Total_path_length_m', ...
...
overallFormats = {'%.2f','%.2f','%.3f','%.2f','%.2f','%.2f','%.4f','%.0f'};
...
if showTables
    ...
    fprintf('Tổng quan | Theta_b = %.2f deg | Theta_d = %.2f deg | Tổng cost = %.3f | Tổng chiều dài = %.2f m | Diện tích vùng = %.2f m^2 | Diện tích quét = %.2f m^2 | Overlap ratio = %.4f | Tổng số lần quay = %.0f\n', ...
        rad2deg(bestConfig.thetaBase), rad2deg(bestConfig.thetaDiag), bestConfig.totalCost, sum(path_length), totalRegionArea, sum(covered_area), overlap_ratio, sum(num_turns));
end

if showPlots
    columnFormatCharUAV = repmat({'char'},1,size(uavMetricsArray,2));
    columnFormatCharOverall = repmat({'char'},1,size(overallArray,2));
    statsFig = figure('Color','w','Name','Hình 3 – Thống kê số liệu Diagonal');
...
Result = struct( ...
    'algorithm_name', 'Diagonal lawn-mower', ...
    'numUAV', numUAV, ...
    'theta_opt', thetaInfo, ...
    'paths', {paths}, ...
...
fprintf('[Diagonal] Tổng cost = %.3f, Tổng chiều dài = %.1f m, mean turns = %.2f\n', ...
        Result.total_cost, sum(path_length), mean(num_turns));
...
function path = generate_diagonal_lawnmower(polyRot, spacing, thetaDiag)
    if isempty(polyRot.Vertices)
        path = [];
        return;
    end
    [cx, cy] = centroid(polyRot);
    polyShifted = translate(polyRot, -cx, -cy);
    polyDiagFrame = rotate(polyShifted, -rad2deg(thetaDiag), [0 0]);
    pathDiagFrame = generate_standard_lawnmower(polyDiagFrame, spacing);
    pathShifted = rotate_points(pathDiagFrame, thetaDiag);
    path = pathShifted + [cx cy];
end

function path = generate_standard_lawnmower(polyRot, spacing)
    verts = polyRot.Vertices;
    verts = verts(~any(isnan(verts),2),:);
    if isempty(verts)
        path = [];
        return;
    ...
    yLines = yMin:spacing:yMax;
    if yLines(end) < yMax
        yLines(end+1) = yMax;
    end
    path = [];
    for k = 1:numel(yLines)
        segments = horizontal_intersections(polyRot, yLines(k));
        if isempty(segments)
            continue;
        end
        if mod(k,2) == 1
            segOrder = 1:size(segments,1);
            headFirst = true;
        else
            segOrder = size(segments,1):-1:1;
            headFirst = false;
        end
        linePts = [];
        for idx = segOrder
            seg = segments(idx,:);
            if headFirst
                localPts = [seg(1) yLines(k); seg(2) yLines(k)];
            else
                localPts = [seg(2) yLines(k); seg(1) yLines(k)];
            end
            linePts = [linePts; localPts]; %#ok<AGROW>
        end
        if isempty(path)
            path = linePts;
        else
            path = [path; linePts]; %#ok<AGROW>
        end
    end
end

function segments = horizontal_intersections(polyObj, yLine)
    ...
    for i = 1:numel(vx)-1
        y1 = vy(i); y2 = vy(i+1);
        x1 = vx(i); x2 = vx(i+1);
        if yLine < min(y1,y2) || yLine > max(y1,y2)
            continue;
        end
        if abs(y2 - y1) < 1e-9
            continue;
    ...
    cleaned = xs(1);
    for idx = 2:numel(xs)
        if abs(xs(idx) - cleaned(end)) > 1e-9
            cleaned(end+1) = xs(idx); %#ok<AGROW>
        end
    end
    xs = cleaned;
    if mod(numel(xs),2) ~= 0
