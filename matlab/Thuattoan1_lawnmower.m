function Result = Thuattoan1_lawnmower(showPlots, showTables)
...
if showPlots
    ...
    title('Hình 1 – Phân vùng quét trong hệ ENU');
end
...
%% --- STEP 4: Xây dựng thuật toán lawn-mower ---
numUAV = 6;
camRadius = 10;           % bán kính footprint (m)
maxOverlap = 5;           % overlap tối đa giữa hai dải (m)
sweepSpacing = 2*camRadius - maxOverlap; % khoảng cách giữa các đường quét
thetaStep = deg2rad(5);   % bước quét góc (rad)
thetaList = 0:thetaStep:(pi - thetaStep); % 0 → gần pi
costWeights = [1.0, 0.02, 0.5]; % [alpha, beta, gamma]
basePoint = [0 0];
costTemplate = struct('turn',0,'length',0,'overlap',0,'total',0,'sweptArea',0,'turnCount',0);

bestCost = inf;
bestTheta = NaN;
bestPaths = cell(numUAV,1);
bestCosts = repmat(costTemplate, numUAV,1);
bestSubPolysENU = cell(numUAV,1);

for theta = thetaList
    rotP = rotate(P, -rad2deg(theta), [0 0]);
    ...
    for k = 1:numUAV
        pathRot = generate_lawnmower_path(subPolysRot{k}, sweepSpacing);
        if size(pathRot,1) < 2
            validConfig = false;
            break;
        end
        scanPathENU = rotate_points(pathRot, theta);
        pathWithTransit = add_transit_segments(scanPathENU, basePoint);
        subPolyENU = rotate(subPolysRot{k}, rad2deg(theta), [0 0]);
        costStruct = evaluate_lawnmower_cost(pathWithTransit, scanPathENU, subPolyENU, camRadius, costWeights);
    ...
    fprintf('Theta = %5.1f deg → Tổng chi phí = %.3f\n', rad2deg(theta), thetaTotal);

    if thetaTotal < bestCost
        bestCost = thetaTotal;
        bestTheta = theta;
        bestPaths = thetaPaths;
        bestCosts = thetaCosts;
        bestSubPolysENU = cellfun(@(poly) rotate(poly, rad2deg(theta), [0 0]), ...
                                  subPolysRot, 'UniformOutput', false);
    end
end

if ~isfinite(bestCost)
    error('Không tìm được cấu hình lawn-mower hợp lệ cho bất kỳ θ nào.');
end

fprintf('\nHướng tối ưu θ* = %.2f deg, tổng chi phí = %.3f\n', rad2deg(bestTheta), bestCost);
...
if showPlots
    figure('Color','w'); hold on; grid on; axis equal; box on;
    plot(P,'FaceColor',[0.9 0.95 1.0],'FaceAlpha',0.3,'EdgeColor',[0.2 0.4 0.8],'LineWidth',1.2);
    colors = lines(numUAV);
    legendEntries = {'Vùng quét gốc'};
    for k = 1:numUAV
        subPoly = bestSubPolysENU{k};
    ...
    xlabel('East (m)'); ylabel('North (m)');
    title(sprintf('Hình 2 – Quỹ đạo lawn-mower tối ưu (θ* = %.1f°)', rad2deg(bestTheta)));
    legend(legendEntries, 'Location','bestoutside');
end

turnCost = [bestCosts.turn]';
lenCost = [bestCosts.length]';
overlapCost = [bestCosts.overlap]';
totalCost = [bestCosts.total]';
areaSwept = [bestCosts.sweptArea]';
turnCount = [bestCosts.turnCount]';
if showTables
    fprintf('Bảng chi tiết cho từng UAV được hiển thị bên dưới.\n');
end

%% --- STEP 5: Chuẩn hóa kết quả ---
pathLengthFull = zeros(numUAV,1);
for k = 1:numUAV
    pathLengthFull(k) = path_length_from_points(bestPaths{k});
end
regionAreas = cellfun(@area, bestSubPolysENU);
totalRegionArea = sum(regionAreas);
paths = bestPaths;
partitions = bestSubPolysENU;
...
overallTable = table(rad2deg(bestTheta), bestCost, sum(path_length), totalRegionArea, ...
...
if showTables
    ...
    fprintf('Tổng quan | Theta* = %.2f deg | Tổng cost = %.3f | Tổng chiều dài = %.2f m | Diện tích vùng = %.2f m^2 | Diện tích quét = %.2f m^2 | Overlap ratio = %.4f | Tổng số lần quay = %.0f\n', ...
        rad2deg(bestTheta), bestCost, sum(path_length), totalRegionArea, sum(covered_area), overlap_ratio, sum(num_turns));
end

if showPlots
    columnFormatCharUAV = repmat({'char'},1,size(uavMetricsArray,2));
    columnFormatCharOverall = repmat({'char'},1,size(overallArray,2));
    statsFig = figure('Color','w','Name','Hình 3 – Thống kê số liệu UAV');
...
Result = struct( ...
    'algorithm_name', 'Lawn-mower', ...
    'numUAV', numUAV, ...
    'theta_opt', rad2deg(bestTheta), ...
    'paths', {paths}, ...
...
fprintf('[Lawn-mower] Tổng cost = %.3f, Tổng chiều dài = %.1f m, mean turns = %.2f\n', ...
...
function path = generate_lawnmower_path(polyRot, spacing)
...
function costStruct = evaluate_lawnmower_cost(fullPath, scanPath, polyRegion, camRadius, weights)
