function Result = Thuattoan2_quetxoanoc(showPlots, showTables)
...
%% --- STEP 4: Tối ưu đường xoắn ốc (Spiral sweep) ---
numUAV = 6;
...
basePoint = [0 0];
costWeights = [1.2, 0.02, 0.6]; % nhấn mạnh chi phí quay đầu hơn lawn-mower
...
for theta = thetaList
    ...
    for k = 1:numUAV
        spiralPathRot = generate_spiral_path(subPolysRot{k}, sweepSpacing);
        if size(spiralPathRot,1) < 2
            validConfig = false;
            break;
        end
        spiralPathENU = rotate_points(spiralPathRot, theta);
        pathWithTransit = add_transit_segments(spiralPathENU, basePoint);
        subPolyENU = rotate(subPolysRot{k}, rad2deg(theta), [0 0]);
        costStruct = evaluate_sweep_cost(pathWithTransit, spiralPathENU, subPolyENU, camRadius, costWeights);
    ...
    fprintf('Theta = %5.1f deg → Tổng chi phí spiral = %.3f\n', rad2deg(theta), thetaTotal);
...
if ~isfinite(bestCost)
    error('Không tìm được cấu hình spiral hợp lệ cho bất kỳ θ nào.');
end

fprintf('\nHướng tối ưu θ* (spiral) = %.2f deg, tổng chi phí = %.3f\n', rad2deg(bestTheta), bestCost);
...
if showPlots
    figure('Color','w'); hold on; grid on; axis equal; box on;
    plot(P,'FaceColor',[0.95 0.9 1.0],'FaceAlpha',0.25,'EdgeColor',[0.4 0.2 0.6],'LineWidth',1.2);
    colors = lines(numUAV);
    ...
    xlabel('East (m)'); ylabel('North (m)');
    title(sprintf('Hình 2 – Spiral sweep tối ưu (θ* = %.1f°)', rad2deg(bestTheta)));
...
%% --- STEP 5: Chuẩn hóa kết quả spiral ---
...
if showPlots
    ...
    statsFig = figure('Color','w','Name','Hình 3 – Thống kê số liệu Spiral');
...
fprintf('[Spiral] Tổng cost = %.3f, Tổng chiều dài = %.1f m, mean turns = %.2f\n', ...
...
function path = generate_spiral_path(polyRot, spacing)
    path = [];
    if isempty(polyRot.Vertices)
        return;
    end
    loops = {};
    current = clean_loop(polyRot);
    maxLoops = 200;
    minArea = max(1.0, 0.0005*area(polyRot));
    for k = 1:maxLoops
        if isempty(current.Vertices)
            break;
        end
        loops{end+1} = current; %#ok<AGROW>
        buffered = polybuffer(current, -spacing);
        nextPoly = select_largest_component(buffered);
        if isempty(nextPoly.Vertices) || area(nextPoly) < minArea
            break;
        end
        current = nextPoly;
    end
    ...
    innerPoly = loops{end};
    [xc, yc] = centroid(innerPoly);
    anchor = [xc yc];
    path = anchor;
    currPoint = anchor;
    tol = 1e-6;
    for idx = numel(loops):-1:1
        loopPoly = loops{idx};
        boundary = ordered_boundary(loopPoly, anchor);
        if isempty(boundary)
            continue;
        end
        entryPt = boundary(1,:);
        if norm(currPoint - entryPt) > tol
            path = [path; entryPt]; %#ok<AGROW>
        end
        path = [path; boundary(2:end,:)]; %#ok<AGROW>
        currPoint = path(end,:);
        anchor = entryPt;
    end
...
function polyOut = clean_loop(polyIn)
    if isempty(polyIn.Vertices)
        polyOut = polyshape();
    else
        polyOut = polyshape(polyIn.Vertices);
    end
end

function boundary = ordered_boundary(polyObj, anchor)
    ...
    dists = vecnorm(verts - anchor, 2, 2);
    [~, idx] = min(dists);
    boundary = [verts(idx:end,:); verts(1:idx,:); verts(idx,:)];
end
