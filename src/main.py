...
def run_commands():
    ...
    commands = [
        ...
        # f"python3 src/mavsdk_server_shell.py {PROTO}://{SERVER_HOST}:{DEFAULT_BIND_PORT + 0} -p {DEFAULT_PORT + 0} -i 1",
        # f"python3 src/mavsdk_server_shell.py {PROTO}://{SERVER_HOST}:{DEFAULT_BIND_PORT + 1} -p {DEFAULT_PORT + 1} -i 2",
        # f"python3 src/mavsdk_server_shell.py {PROTO}://{SERVER_HOST}:{DEFAULT_BIND_PORT + 2} -p {DEFAULT_PORT + 2} -i 3",
        # f"python3 src/mavsdk_server_shell.py {PROTO}://{SERVER_HOST}:{DEFAULT_BIND_PORT + 3} -p {DEFAULT_PORT + 3} -i 4",
        # f"python3 src/mavsdk_server_shell.py {PROTO}://{SERVER_HOST}:{DEFAULT_BIND_PORT + 4} -p {DEFAULT_PORT + 4} -i 5",
        # f"python3 src/mavsdk_server_shell.py {PROTO}://{SERVER_HOST}:{DEFAULT_BIND_PORT + 5} -p {DEFAULT_PORT + 5} -i 6",
        "python3 src/app.py",
