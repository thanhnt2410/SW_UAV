...
class UAVAnalyzer:
    ...
    def _polygon_xy_to_geojson(self, geom) -> Dict[str, Any]:
        """
        Chuyển 1 hình học shapely (Polygon/MultiPolygon) trong hệ tọa độ mét
        (UTM) trở lại hệ GPS (lat, lon) và xuất ra dạng GeoJSON.
        """
        if geom.is_empty:
    ...
    def _iter_polygons(geom):
        """Duyệt qua từng Polygon con, dù geom là Polygon hay MultiPolygon."""
        if geom.is_empty:
            return
    ...
    def visualize(
        ...
        """
        Vẽ bản đồ bao phủ (vùng khảo sát, quỹ đạo bay, vùng đã quét) bằng
        matplotlib và trả về đối tượng Figure.
 
        Args:
            title: Tiêu đề biểu đồ.
            show_waypoints: Nếu True, đánh dấu từng waypoint trên quỹ đạo.
            figsize: Kích thước hình (inch).
            save_path: Nếu được cung cấp, lưu hình ra file (vd: "coverage.png").
            show: Nếu True, gọi plt.show() để hiển thị hình ngay.
 
        Returns:
            matplotlib.figure.Figure đã vẽ.
 
        Raises:
            ImportError: Nếu chưa cài matplotlib.
        """
        try:
        ...
        # Chú thích: dùng proxy artist (KHÔNG add_patch vào ax) để tránh làm
        # sai lệch dataLim/autoscale của trục (nếu add_patch 1 hình tại tọa
        # độ giả như (0,0), trục sẽ tự phóng to để bao luôn điểm đó, làm dữ
        # liệu thật ở tọa độ UTM hàng trăm nghìn mét bị co lại thành 1 chấm).
        legend_handles = [
